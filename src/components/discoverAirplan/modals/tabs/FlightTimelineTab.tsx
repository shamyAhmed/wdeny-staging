...
import { FlightJourney, FlightSegment } from "@/app/[locale]/_types/FlightOffer";
...
const getJourneyDurationMinutes = (journey: FlightJourney) => {
  ...
  return dayjs(last.arrivalDateTime).diff(dayjs(first.departureDateTime), "minute");
};
  ...
  return (
    <div className="flight-timeline">
      {journeys.map((journey, journeyIdx) => {
        ...
        const routeTitle = `${journey.origin} إلى ${journey.destination}`;
        const dateLabel = dayjs(firstSeg.departureDateTime).format("DD MMMM YYYY");
        const totalDuration = formatDuration(getJourneyDurationMinutes(journey));

        return (
          <div
            key={journey.id}
            className={journeyIdx > 0 ? "mt-10 pt-10 border-t border-dashed border-gray-300" : ""}
          >
            {/* Journey header */}
            <div className="mb-8 border-b border-[#e9e9e9] pb-4">
              ...
              <div className="flex items-center justify-between text-[#888] text-sm">
                <span>{dateLabel}</span>
                <span>المدة الإجمالية: {totalDuration}</span>
              </div>
            </div>
            ...
            <div className="space-y-7 pb-7">
              {journey.segment.map((seg, idx) => {
                ...
                const departDate = dayjs(seg.departureDateTime).format("DD MMM");
                const arriveDate = dayjs(seg.arrivalDateTime).format("DD MMM");
                const flightDuration = formatDuration(seg.flightTimeInMinutes);
                const fromTerminal = seg.departureTerminal ? `صالة المطار ${seg.departureTerminal}` : "";
                const toTerminal = seg.arrivalTerminal ? `صالة المطار ${seg.arrivalTerminal}` : "";
                const isLastSegment = idx === journey.segment.length - 1;
                const nextSeg = journey.segment[idx + 1];
                const layoverMinutes = !isLastSegment ? getLayoverMinutes(seg, nextSeg) : 0;
                  ...
                  <div key={seg.id}>
                    <div className="grid grid-cols-[70px_54px_1fr] sm:grid-cols-[84px_62px_1fr] gap-2 sm:gap-4">
                      ...
                      <div className="flex flex-col justify-between text-right min-h-[188px] py-1">
                        <div>
                          <p className="text-xl leading-none text-[#666]">{departTime}</p>
                          <p className="text-xs text-[#9b9b9b]">{departDate}.</p>
                        </div>
                        <div>
                          <p className="text-xl leading-none text-[#666]">{arriveTime}</p>
                          <p className="text-xs text-[#9b9b9b]">{arriveDate}.</p>
                        </div>
                      </div>
                      ...
                      <div className="text-right space-y-5">
                        <div>
                          ...
                          {fromTerminal && (
                            <p className="text-xs text-[#9b9b9b]">{fromTerminal}</p>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-5 py-1">
                          ...
                          <div className="text-[11px] sm:text-xs text-[#8d8d8d] leading-6 text-right">
                            ...
                            {seg.equipment && <p>الطائرة: {seg.equipment}</p>}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          {toTerminal && (
                            <p className="text-xs text-[#9b9b9b]">{toTerminal}</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    {!isLastSegment && (
                      <div className="bg-[#edd0d0] rounded-xl text-center text-sm text-[#6b6b6b] py-3 mt-5">
                        وقت الترانزيت:  {formatDuration(layoverMinutes)}
                      </div>
                    )}
                  </div>
                );
